...
def _apply_park_transform(sample):
    ...
    v_2_ch = 5
    v_3_ch = 6
    i_alpha = (2 * sample[:i_1_ch, :] -
               sample[i_1_ch:i_2_ch, :] - sample[i_2_ch:i_3_ch, :]) / 3
    i_beta = (sample[i_1_ch:i_2_ch, :] - sample[i_2_ch:i_3_ch, :]) / np.sqrt(3)
    v_alpha = (2 * sample[i_3_ch:v_1_ch, :] -
               sample[v_1_ch:v_2_ch, :] - sample[v_2_ch:v_3_ch, :]) / 3
...
def park_transform(input_data: Union[InputData, np.ndarray]) -> np.ndarray:
    ...
    features = input_data.features if isinstance(
        input_data, InputData) else input_data
