...
class StatisticalDetector(AnomalyDetector):
    ...
    def __init__(self, params: Optional[OperationParameters] = None):
        ...
        self.node_list = self.params.get(
            'node_list', ['quantile_extractor', 'one_class_svm'])
    ...
    def score_samples(self, input_data: InputData) -> np.ndarray:
        ...
        features = self.model_impl.nodes[1].fitted_operation.transform(
            converted_input_data).predict
        scores = self.model_impl.nodes[0].fitted_operation.score_samples(
