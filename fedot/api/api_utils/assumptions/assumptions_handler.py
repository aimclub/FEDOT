...
class AssumptionsHandler:
    def __init__(self, data: Union[InputData, TensorData]):
        """
    ...
    def propose_assumptions_with_tensordata(
            self,
            initial_assumption: Union[List[Pipeline], Pipeline, None]) -> List[Pipeline]:
        """Return user-provided initial assumptions for TensorData.
        ...
        if not normalized:
            raise ValueError('Initial assumption should contain at least one pipeline')
        return normalized
