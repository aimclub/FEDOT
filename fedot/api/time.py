...
class ApiTime:
    ...
    def have_time_for_tuning(self):
        ...
        # The tuner evaluates the initial graph before starting optimisation.
        # Keep enough time both for this CV evaluation and for a useful search.
        minimum_tuning_time = (
            self.assumption_fit_spend_time.total_seconds()
            + MINIMAL_SECONDS_FOR_TUNING
        )
        return timeout_for_tuning >= minimum_tuning_time
    ...
    def determine_resources_for_tuning(self):
        ...
        how much time is available for tuning.

        A full-data refit is performed after model generation, and GOLEM's
        tuner performs a final cross-validation check after the timed search.
        Reserve the estimated cost of both stages here so that tuning cannot
        consume the entire AutoML budget before them.

        """
        all_spend_time = self.composing_spend_time + self.assumption_fit_spend_time_single_fold
        reserved_time = self.assumption_fit_spend_time_single_fold + self.assumption_fit_spend_time
        ...
        if self.time_for_automl is not None:
            ...
            timeout_for_tuning = (
                timeout_in_sec
                - all_spend_time.total_seconds()
                - reserved_time.total_seconds()
            )
        else:
