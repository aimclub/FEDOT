...
class PipelineEnv(gym.Env):
    ...
    def __init__(self, path_to_data=None, path_to_valid=None, env_name='default', mode='train', pipeline_depth=4,
                 graph_render=True):
        ...
        if self.mode == 'train':
            ...
            if action == self.eop or self.cur_pos == len(self.observation):
                ...
                try:
                    ...
                    rwd = 100 * self.metric_value
                    self.render(mode='graph')
                    reward, done, info = self._env_response(reward=rwd, length=len(self.nodes))
                    return self.observation, reward, done, info
                except ValueError:
        ...
        if self.mode == 'test':
            ...
            if action == self.eop or self.cur_pos == len(self.observation):
                ...
                try:
                    ...
                    self.metric_value = roc_auc(y_true=self.validing_data, y_score=results.predict, multi_class='ovo',
                                                average='macro')
