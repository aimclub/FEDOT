...
class ExecutionContext:
    ...
    def __getstate__(self):
        return {
            ...
            "extension_state": getattr(self.extension, "get_state", lambda: None)(),
    ...
    def __setstate__(self, state):
        ...
        self.backend = state.get("backend", "default")
        self.class_rules = state["class_rules"]
        self.ts_rules = state["ts_rules"]
        self.common_rules = state["common_rules"]

        if state.get("has_extension", False):
            self.extension = IndustrialExtension(backend=self.backend)

            ext_state = state.get("extension_state")
            if ext_state and hasattr(self.extension, "set_state"):
                self.extension.set_state(ext_state)
