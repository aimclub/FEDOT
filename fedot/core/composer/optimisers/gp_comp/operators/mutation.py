...
def get_mutation_prob(mut_id, root_node):
    ...
    if mut_id in list(MutationStrengthEnum):
        ...
        mutation_prob = mutation_strength / (GraphOperations().distance_to_leaf_level(root_node) + 1)
    else:
...
def growth_mutation(chain: Any, requirements, chain_generation_params, max_depth: int, local_growth=True) -> Any:
    ...
    if local_growth:
        ...
        is_primary_node_selected = randint(0, 1) and not chain.operations.distance_to_root_level(node_from_chain) < max_depth
    if is_primary_node_selected:
        new_subtree = chain_generation_params.primary_node_func(operation_type=choice(requirements.primary))
    else:
        if local_growth:
            max_depth = GraphOperations().distance_to_leaf_level(node_from_chain)
        else:
            # max_depth = max_depth - node_height(chain, node_from_chain)
            max_depth = max_depth - chain.operations.distance_to_root_level(node_from_chain)
    ...
    chain.update_node(node_from_chain, new_subtree, include_parents=True)
    return chain
...
def reduce_mutation(chain: Any, requirements, chain_generation_params, max_depth: int = None) -> Any:
    ...
    children = chain.operations.node_children(node_to_del)
    is_possible_to_delete = all([len(child.nodes_from) - 1 >= requirements.min_arity for child in children])
    if is_possible_to_delete:
        chain.delete_node(node_to_del, include_parents=True)
    else:
        primary_node = chain_generation_params.primary_node_func(operation_type=choice(requirements.primary))
        chain.update_node(node_to_del, primary_node, include_parents=True)
    return chain
