...
class HistoryVisualization(metaclass=ArgConstraintWrapper):
    ...
    The only necessary method is 'visualize' - it must show or save the plot in any form
    after the call.

    One should refer the OptHistory instance as self.history() to be able to connect one's
    visualization to OptHistory.show(). See the examples of connecting visualizations in the module
    opt_viz.py.

    It is good practice to be aware of constraints on your visualizations. You can either implement
    default constraints that will catch your kwarg across all the visualizations
    or define your single class specific constraints by assigning them to `constraint_checkers` class attribute.
