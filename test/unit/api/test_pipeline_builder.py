...
def test_pipeline_builder():
    ...
    assert pipelines_same(Pipeline(n2b), PipelineBuilder(
        SecondaryNode('ridge', nodes_from=[PrimaryNode('lagged')])).to_pipeline())  # single node

    # build single node
    assert pipelines_same(Pipeline(n1a), PipelineBuilder().node('glm').to_pipeline())  # single node
    assert pipelines_same(Pipeline(n1a),
                          PipelineBuilder().node('glm', branch_idx=42).to_pipeline())  # index overflow is ok
    ...
    assert pipelines_same(Pipeline([n1a, n1b]),
                          PipelineBuilder().node('glm').node('lagged', branch_idx=1).to_pipeline())
    assert pipelines_same(Pipeline([n12a, n2b]),
                          PipelineBuilder().branch('glm', 'lagged').grow_branches('ridge', 'ridge').to_pipeline())
    ...
    # joined branch
    assert pipelines_same(Pipeline([n12a, n12b]),
                          PipelineBuilder().node('glm').branch('glm', 'ridge').join(None).to_pipeline())  # no-op join
    assert pipelines_same(Pipeline(n123a),
                          PipelineBuilder().node('glm').branch('glm', 'ridge').join('ridge').to_pipeline())
