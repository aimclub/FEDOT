...
def test_change_node(nodes, node_factory):
    ...
    assert new_intermediate_node.content['name'] in node_factory.graph_model_repository.get_operations(is_primary=False) and new_intermediate_node.content['name'] != intermediate_node.content['name']
...
def test_get_intermediate_parent_node(nodes, node_factory):
    ...
    assert new_intermediate_parent_node.content['name'] in node_factory.graph_model_repository.get_operations(is_primary=False)
    assert new_intermediate_parent_node.content['name'] != secondary_node.content['name']
    assert new_intermediate_parent_node.content['name'] not in [str(n.content['name']) for n in secondary_node.nodes_from]
...
def test_get_separate_parent_node(nodes, node_factory):
    ...
    assert new_separate_parent_node.content['name'] in node_factory.graph_model_repository.get_operations(is_primary=True)
