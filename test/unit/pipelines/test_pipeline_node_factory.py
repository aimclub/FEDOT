...
def test_change_node(nodes, node_factory):
    ...
    assert new_primary_node.content['name'] in node_factory.graph_model_repository.get_operations(
        is_primary=True)
    ...
    assert new_secondary_node.content['name'] in node_factory.graph_model_repository.get_operations(
        is_primary=False)


def test_get_intermediate_parent_node(nodes, node_factory):
    _, _, secondary_node = nodes
    new_intermediate_parent_node = node_factory.get_parent_node(
        secondary_node, is_primary=False)
...
def test_get_separate_parent_node(nodes, node_factory):
    _, _, secondary_node = nodes
    new_separate_parent_node = node_factory.get_parent_node(
        secondary_node, is_primary=True)
...
def test_get_child_node(node_factory):
    ...
    assert new_child_node.content['name'] in node_factory.graph_model_repository.get_operations(
        is_primary=False)
