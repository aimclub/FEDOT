...
from data.pipeline_manager import pipeline_first, pipeline_third, pipeline_second, pipeline_fourth
...
def test_equality_cases():
    list_pipelines_pairs = equality_cases()
    for pair in list_pipelines_pairs:
        assert pair[0] == pair[1]
        assert pair[1] == pair[0]


def test_non_equality_cases():
    list_pipelines_pairs = non_equality_cases()
    for pair in list_pipelines_pairs:
