...
def params():
    ...
    ts_test = np.genfromtxt(
        f'{fedot_project_root()}/test/unit/pipelines/prediction_intervals/data/test_ts.csv')
...
def test_prediction_intervals(params):
    pred_ints_params = PredictionIntervalsParams(
        number_mutations=80, bpq_number_models='max', show_progress=False)
    for x in ['mutation_of_best_pipeline', 'best_pipelines_quantiles']:

        pred_ints = PredictionIntervals(
            model=params['model'], method=x, horizon=20, params=pred_ints_params)
        pred_ints.fit(params['train_input'])
        res = pred_ints.forecast()

        int_score = interval_score(
            params['ts_test'], low=res['low_int'], up=res['up_int'])
        int_picp = picp(params['ts_test'],
                        low=res['low_int'], up=res['up_int'])
