...
# ------------------------------------------------------------------------------
# Tests fro disconnect_nodes method in GraphOperator
...
def test_disconnect_nodes_method():
    pipeline = get_initial_pipeline()

    # Disconnect knn_node_third and knn_root nodes
    res_pipeline = get_res_pipeline_test1()

    pipeline1 = deepcopy(pipeline)
    knn_node = pipeline1.nodes[4]
    knn_root_node = pipeline1.nodes[0]

    pipeline1.operator.disconnect_nodes(knn_node, knn_root_node)

    assert(res_pipeline == pipeline1)

    # Disconnect xgb_node and knn_node_third
    res_pipeline2 = get_res_pipeline_test2()

    pipeline2 = deepcopy(pipeline)
    xgboost_node = pipeline2.nodes[5]
    knn_node = pipeline2.nodes[4]

    pipeline2.operator.disconnect_nodes(xgboost_node, knn_node)

    assert(res_pipeline2 == pipeline2)

    # Disconnect qda_node and knn_root nodes
    res_pipeline3 = get_res_pipeline_test3()

    pipeline3 = deepcopy(pipeline)
    qda_node = pipeline3.nodes[1]
    knn_root_node = pipeline3.nodes[0]

    pipeline3.operator.disconnect_nodes(qda_node, knn_root_node)

    assert(res_pipeline3 == pipeline3)

    # Try to disconnect nodes between which there is no edge
    pipeline4 = deepcopy(pipeline)

    xgboost_node = pipeline4.nodes[2]
    knn_root_node = pipeline4.nodes[0]
    pipeline4.operator.disconnect_nodes(xgboost_node, knn_root_node)
    assert (pipeline4 == pipeline)

    # Try to disconnect nodes that are not in this pipeline
    pipeline5 = deepcopy(pipeline)
    pipeline5.operator.disconnect_nodes(xgboost_node, knn_root_node)
    assert (pipeline5 == pipeline)
