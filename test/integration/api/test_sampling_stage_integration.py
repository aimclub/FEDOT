...
class StratifiedStubProvider(SamplingProvider):
    def sample(self,
        ...
        for label in np.unique(target):
            ...
            picked = rng.choice(label_idx, size=min(
                k, len(label_idx)), replace=False)
...
def test_sampling_stage_skipped_when_predefined_model(monkeypatch):
    ...
    def should_not_run_stage(self):
        raise AssertionError(
            'sampling stage must be skipped for predefined_model')

    monkeypatch.setattr(
        Fedot, '_run_sampling_stage_if_necessary', should_not_run_stage)
    ...
    assert pipeline is not None
    assert model.sampling_stage_metadata == {
        'status': 'skipped', 'reason': 'predefined_model'}
