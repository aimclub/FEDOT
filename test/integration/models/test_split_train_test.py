...
def get_roc_auc_value(pipeline: Pipeline, train_data: InputData, test_data: InputData) -> Tuple[float, float]:
    ...
    roc_auc_value_test = roc_auc(
        y_true=test_data.target, y_score=test_pred.predict)
    roc_auc_value_train = roc_auc(
        y_true=train_data.target, y_score=train_pred.predict)
...
def get_random_target_data(data: InputData) -> InputData:
    ...
    data_copy.target = np.array([random.choice((0, 1))
...
def test_model_fit_correctly_but_predict_incorrectly():
    ...
    train_data = get_synthetic_input_data(
        N_SAMPLES, N_FEATURES, random_state=1)
