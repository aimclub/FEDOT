...
from core.composer.optimisers.selection import tournament_selection, individuals_selection, random_selection, selection
...
def rand_population_generation_and_eval(pop_size):
...
def obj_function(train_data: InputData, test_data: InputData, chain: Chain) -> float:
    metric_function = MetricsRepository().metric_by_id(ClassificationMetricsEnum.ROCAUC)
    chain.fit(input_data=train_data)
    return metric_function(chain, test_data)


population = rand_population_generation_and_eval(pop_size=4)
num_of_inds = 2


def test_tournament_selection():
    selected_individuals = tournament_selection(individuals=population, pop_size=num_of_inds)
    assert all([ind in population for ind in selected_individuals]) and len(selected_individuals) == num_of_inds


def test_random_selection():
    selected_individuals = random_selection(individuals=population, pop_size=num_of_inds)
    assert all([ind in population for ind in selected_individuals]) and len(selected_individuals) == num_of_inds


def test_selection():
    selected_individuals = selection(types=[SelectionTypesEnum.tournament], population=population, pop_size=num_of_inds)
    assert all([ind in population for ind in selected_individuals]) and len(selected_individuals) == num_of_inds
...
def test_individuals_selection():
    ...
    assert len(set(selected_individuals_ref)) == len(selected_individuals) and len(selected_individuals) == num_of_inds
