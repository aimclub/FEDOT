...
class ModelGapFiller(SimpleGapFiller):
    ...
    def forward_inverse_filling(self, input_data, exog_array):
        """
    ...
    def forward_filling(self, input_data, exog_array):
        ...
        for gap in new_gap_list:
            ...
            # Get array with exogenous data
            exog_train_part = exog_array[:gap[0]]
            exog_test_part = exog_array[gap]
    ...
    def __chain_fit_predict(self, timeseries_train: np.array, exog_train_part: np.array,
                            exog_test_part: np.array, len_gap: int):
        """
...
def run_fedot_ridge(folder_to_save, files_list, exog_files_list,
    ...
    :param exog_files_list: list with file names, which are exog features
    ...
    for file_id, file in enumerate(files_list):
        ...
        # Exog file
        exog_file = exog_files_list[file_id]
        exog_data = pd.read_csv(os.path.join('..', 'data', exog_file))
        exog_data['Date'] = pd.to_datetime(exog_data['Date'])

        if vis == True:
            plt.plot(np.array(data['Height']))
            plt.plot(np.array(exog_data['Height']), label='Exog')
        ...
        for column_with_gap in columns_with_gap:
            ...
            exog_array = np.array(exog_data['Height'])
            ...
            gapfiller = ModelGapFiller(gap_value=-100.0,
                                       chain=ridge_chain,
                                       max_window_size=20)
            withoutgap_arr = gapfiller.forward_filling(array_with_gaps, exog_array)
...
folder_to_save = 'D:/iccs_article/fedot_ridge_exog'
files_list = ['Sea_10_240.csv']
exog_files_list = ['Sea_10_239.csv']
columns_with_gap = ['gap', 'gap_center']
file_with_results = 'D:/iccs_article/fedot_ridge_exog_report.csv'

if __name__ == '__main__':
    run_fedot_ridge(folder_to_save, files_list, exog_files_list,
