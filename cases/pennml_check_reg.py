...
def run_classification_exp(dataset_numbers: List[int], clip_data: bool = False,
    ...
    """
    data_path = 'D:/1 Работа/Датасеты/reg'
    ...
    for dataset_number in dataset_numbers:
        ...
        if dataset_name == 'used_car.csv':
            ...
            id_target = np.ravel(np.argwhere(column_names == 'price'))
        ...
        pipeline = fedot.fit(features=train_data, predefined_model='auto')
...
if __name__ == '__main__':
    dataset_number = 3
    run_classification_exp(dataset_numbers=[dataset_number], clip_data=True, safe_mode=False)
    run_classification_exp(dataset_numbers=[dataset_number], clip_data=False, safe_mode=False)
    run_classification_exp(dataset_numbers=[dataset_number], clip_data=False, safe_mode=True)
