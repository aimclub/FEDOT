...
def fixed_chain(nodes: list) -> Chain:
    ...
    Max chain depth is equal to 1.

    :param nodes: list of model_id strings from which the chain will be constructed.
    :return: Chain
    """
    if len(nodes) > 1:
...
def run_grasp_robustness_problem(dataset_path, list_of_nodes: list,
    ...
    :param list_of_nodes: list of model_id strings or a single model_id string
        ...
        the others are his children. Max chain depth is equal to 1.
    ...
    if not full_dataset:
        chain_evo_composed.fit(input_data=dataset_train_fit, verbose=True)
    else:
