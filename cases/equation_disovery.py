...
from core.repository.model_types_repository import ModelGroupsIdsEnum
...
def compare_plot(predicted: OutputData, dataset_to_validate: InputData):
    fig, ax = plt.subplots()
    plt.scatter(x=range(len(dataset_to_validate.target)),
                y=dataset_to_validate.target, linewidth=1, label="Observed")
    plt.scatter(x=range(len(predicted.predict)),
                y=predicted.predict, linewidth=1, label="Predicted")
    ax.legend()
...
available_model_types, _ = models_repo.search_models(
    desired_ids=[ModelGroupsIdsEnum.algebraic],
...
# the quality assessment for the obtained algebraic models
