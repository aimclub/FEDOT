...
def run_classification_example(
    ...
    train_data_path = f'{fedot_project_root()}/test/data/classification_with_categorical.csv'
    test_data_path = f'{fedot_project_root()}/test/data/classification_with_categorical.csv'

    # if run_baseline:
        # baseline_model = Fedot(problem=problem, timeout=timeout, use_stats=use_stats)
        # baseline_model.fit(features=train_data_path, target='default', predefined_model='rf')
        #
    ...
    auto_model.fit(features=train_data_path, target='default')
