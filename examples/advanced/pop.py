...
AV_OPS_TEXT = ['adareg', 'dtreg', 'gbr', 'knnreg', 'lasso', 'lgbmreg', 'linear', 'rfr', 'ridge',
               ...
               'ransac_lin_reg', 'ransac_non_lin_reg', 'isolation_forest_reg', 'tfidf']
...
def update_pipeline(pipeline: dict) -> Pipeline:
    ...
    for node in pipeline['data_source_text/article_title'].nodes:
...
def run_multi_modal_example(file_path: str, is_visualise=True, timeout=10) -> float:
    ...
    """
    task = 'regression'
    train_path = Path(fedot_project_root(), file_path, 'news_popularity2_train.csv')
    test_path = Path(fedot_project_root(), file_path, 'news_popularity2_test.csv')
    fit_data = MultiModalData.from_csv(file_path=train_path, task=task, index_col=None,
                                       text_columns=['article_title'],
                                       target_columns='log_shares')
    ...
    #     automl_model.current_pipeline.save(f'pipeline_pop/pipeline_pop_{data_source}', is_datetime_in_path=True)
    #     history_name = data_source.replace('/', '_')
    #     automl_model.history.save(f'pipeline_pop/pipeline_pop_{history_name}.json')
    ...
    automl_model.current_pipeline.save(f'pipeline_pop', is_datetime_in_path=True)
    # automl_model.history.save(f'pipeline_pop/pipeline_pop.json')
...
if __name__ == '__main__':
    metric = run_multi_modal_example(file_path='examples/data/news_popularity2', is_visualise=True, timeout=30)
