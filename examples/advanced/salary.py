...
def update_pipeline(pipeline: dict) -> Pipeline:
    ...
    for node in pipeline['data_source_text/experience'].nodes:
    ...
    for node in pipeline['data_source_text/job_description'].nodes:
        final_pipeline.add_node(node)
    final_pipeline.connect_nodes(final_pipeline.nodes[pos_ens_node
                                                      + len(pipeline['data_source_text/experience'].nodes) + 1],
                                 final_pipeline.nodes[pos_ens_node])
    for node in pipeline['data_source_text/job_desig'].nodes:
        final_pipeline.add_node(node)
    final_pipeline.connect_nodes(final_pipeline.nodes[pos_ens_node
                                                      + len(pipeline['data_source_text/experience'].nodes)
                                                      + len(pipeline['data_source_text/job_description'].nodes) + 1],
                                 final_pipeline.nodes[pos_ens_node])
    for node in pipeline['data_source_text/key_skills'].nodes:
        final_pipeline.add_node(node)
    final_pipeline.connect_nodes(final_pipeline.nodes[pos_ens_node
                                                      + len(pipeline['data_source_text/experience'].nodes)
                                                      + len(pipeline['data_source_text/job_description'].nodes)
                                                      + len(pipeline['data_source_text/job_desig'].nodes) + 1],
                                 final_pipeline.nodes[pos_ens_node])
    for node in pipeline['data_source_text/location'].nodes:
        final_pipeline.add_node(node)
    final_pipeline.connect_nodes(final_pipeline.nodes[pos_ens_node
                                                      + len(pipeline['data_source_text/experience'].nodes)
                                                      + len(pipeline['data_source_text/job_description'].nodes)
                                                      + len(pipeline['data_source_text/job_desig'].nodes)
                                                      + len(pipeline['data_source_text/key_skills'].nodes) + 1],
                                 final_pipeline.nodes[pos_ens_node])
...
def run_multi_modal_example(file_path: str, is_visualise=True, timeout=10) -> float:
    ...
    """
    task = 'classification'
    train_path = Path(fedot_project_root(), file_path, 'data_scientist_salary_train.csv')
    test_path = Path(fedot_project_root(), file_path, 'data_scientist_salary_test.csv')
    fit_data = MultiModalData.from_csv(file_path=train_path, task=task, index_col=None,
                                       text_columns=['experience', 'job_description', 'job_desig', 'key_skills', 'location'],
    ...
    for data_source in fit_data:
        ...
        automl_model = Fedot(problem=task, timeout=1, n_jobs=6,
                             safe_mode=False,
        ...
        metrics = automl_model.get_metrics(metric_names=['acc'])

    #if visualization:
        automl_model.current_pipeline.show(engine='pyvis')
        automl_model.current_pipeline.save(f'pipeline_salary_{data_source}', is_datetime_in_path=True)
        history_name = data_source.replace('/', '_')
        automl_model.history.save(f'pipeline_salary/pipeline_salary_{history_name}.json')
        print(f'Accuracy for {data_source} is {round(metrics["acc"], 3)}')


    final_pipeline = update_pipeline(pipelines_dict)
    automl_model = Fedot(problem=task, timeout=timeout / 2, n_jobs=6, safe_mode=False,
    ...
    automl_model.current_pipeline.save(f'pipeline_salary', is_datetime_in_path=True)
    automl_model.history.save(f'pipeline_salary/pipeline_salary.json')
    print(f'Accuracy for final model is {acc}')
    return metrics["acc"]


if __name__ == '__main__':
    metric = run_multi_modal_example(file_path='examples/data/data_scientist_salary', is_visualise=True, timeout=60)
