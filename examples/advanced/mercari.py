...
AV_OPS_TEXT = ['adareg', 'dtreg', 'gbr', 'knnreg', 'lasso', 'lgbmreg', 'linear', 'ridge',
...
def get_pipeline(fit_data: MultiModalData):
    if fit_data.data_type[0] is DataTypesEnum.text:
        ...
        if node_name in ['data_source_text/name', 'data_source_text/item_description']:
            vect_node = SecondaryNode('tfidf', nodes_from=[primary_node])
            model_node = SecondaryNode('ridge', nodes_from=[vect_node])
        else:
            vect_node = SecondaryNode('cntvect', nodes_from=[primary_node])
            model_node = SecondaryNode('ridge', nodes_from=[vect_node])
    else:
...
def update_pipeline(pipeline: dict) -> Pipeline:
    ...
    for node in pipeline['data_source_text/category_name'].nodes:
        final_pipeline.add_node(node)
    ...
    for node in pipeline['data_source_text/brand_name'].nodes:
        final_pipeline.add_node(node)
    final_pipeline.connect_nodes(final_pipeline.nodes[pos_ens_node
                                                      + len(pipeline['data_source_text/name'].nodes)
                                                      + len(pipeline['data_source_text/category_name'].nodes) + 1],
                                 final_pipeline.nodes[pos_ens_node])
    for node in pipeline['data_source_text/item_description'].nodes:
        final_pipeline.add_node(node)
    final_pipeline.connect_nodes(final_pipeline.nodes[pos_ens_node
                                                      + len(pipeline['data_source_text/name'].nodes)
                                                      + len(pipeline['data_source_text/category_name'].nodes)
                                                      + len(pipeline['data_source_text/brand_name'].nodes) + 1],
                                 final_pipeline.nodes[pos_ens_node])
...
def run_multi_modal_example(file_path: str, is_visualise=True, timeout=10) -> float:
    ...
    """
    task = 'regression'
    train_path = Path(fedot_project_root(), file_path, 'mercari_price_suggestion100K_train.csv')
    test_path = Path(fedot_project_root(), file_path, 'mercari_price_suggestion100K_test.csv')
    fit_data = MultiModalData.from_csv(file_path=train_path, task=task, index_col=None,
                                       ...
                                       columns_to_drop=['train_id', 'price'],
                                       target_columns='log_price')
    ...
    for data_source in fit_data:
        ...
        automl_model = Fedot(problem=task, timeout=timeout_source, n_jobs=1,
                             safe_mode=False,
        ...
        automl_model.current_pipeline.save(f'pipeline_mercari/pipeline_mercari_{data_source}', is_datetime_in_path=True)
        history_name = data_source.replace('/', '_')
        automl_model.history.save(f'pipeline_mercari/pipeline_mercari_{history_name}.json')
    ...
    # path = 'examples/advanced/December-13-2022,14-24-31,PM pipeline_mercari'
    final_pipeline = update_pipeline(pipelines_dict)
    automl_model = Fedot(problem=task, timeout=timeout, n_jobs=1, safe_mode=False,
    ...
    automl_model.current_pipeline.save(f'pipeline_mercari', is_datetime_in_path=True)
    automl_model.history.save(f'pipeline_mercari/pipeline_mercari.json')
    return metrics["r2"]


if __name__ == '__main__':
    metric = run_multi_modal_example(file_path='examples/data/mercari_price_suggestion100K',
