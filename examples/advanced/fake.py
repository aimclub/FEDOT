...
def run_multi_modal_example(file_path: str, timeout=10, visualization=False) -> float:
    ...
    for data_source in fit_data:
        ...
        automl_model.current_pipeline.save(f'pipeline_fake_{data_source}', is_datetime_in_path=True)
        history_name = data_source.replace('/', '_')
        automl_model.history.save(f'pipeline_fake/pipeline_fake_{history_name}.json')
        print(f'ROC-AUC for {data_source} is {round(metrics["roc_auc"], 3)}')

    final_pipeline = update_pipeline(pipelines_dict)
    automl_model = Fedot(problem=task, timeout=180, n_jobs=6, safe_mode=False,
                         initial_assumption=final_pipeline, available_operations=AV_OPS_TEXT, metric='roc_auc')
    print(f'Fitting final model for {180} minutes')
    ...
    automl_model.current_pipeline.save(f'pipeline_fake', is_datetime_in_path=True)
    automl_model.history.save(f'pipeline_fake/pipeline_fake_240.json')
...
if __name__ == '__main__':
    ...
    metric = run_multi_modal_example(file_path='examples/data/fake_job_postings2', timeout=240, visualization=True)
    # history = OptHistory.load('pipeline_fake/pipeline_fake_data_source_table.json')
    # history.show()
    #     res.append(metric)
