...
def get_pipeline(fit_data: MultiModalData):
    if fit_data.data_type[0] is DataTypesEnum.text:
        primary_node = PrimaryNode('data_source_text/article_title')
...
def run_multi_modal_example(file_path: str, timeout=10, visualization=False) -> float:
    ...
    """
    task = 'classification'
    train_path = Path(fedot_project_root(), file_path, 'news_channel_train.csv')
    test_path = Path(fedot_project_root(), file_path, 'news_channel_test.csv')
    ...
    for data_source in fit_data:
        ...
        automl_model = Fedot(problem=task, timeout=timeout_source, n_jobs=6,
                             safe_mode=False, available_operations=AV_OPS_TABLE, metric='acc',
    ...
    automl_model = Fedot(problem=task, timeout=timeout_source, n_jobs=6, safe_mode=False,
                         initial_assumption=final_pipeline, available_operations=AV_OPS_TABLE, metric='acc')
    ...
    metrics = automl_model.get_metrics(metric_names=['acc', 'f1', 'roc_auc'])
    automl_model.current_pipeline.show(engine='pyvis')
    automl_model.current_pipeline.save('pipeline_channel', is_datetime_in_path=True)
    print(f'Accuracy for final model is {round(metrics["acc"], 3)}')
    return metrics["acc"]
...
if __name__ == '__main__':
    # res = []
    # for i in range(5):
    metric = run_multi_modal_example(file_path='examples/data/news_channel', timeout=120, visualization=True)
    #     res.append(metric)
