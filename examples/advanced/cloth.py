...
AV_OPS_TEXT = ['adareg', 'dtreg', 'gbr', 'knnreg', 'lasso', 'lgbmreg', 'linear', 'rfr', 'ridge',
...
def update_pipeline(pipeline: dict) -> Pipeline:
    ...
    for node in pipeline['data_source_text/Review Text'].nodes:
        final_pipeline.add_node(node)
...
def run_multi_modal_example(file_path: str, is_visualise=True, timeout=10) -> float:
    ...
    """
    task = 'regression'
    train_path = Path(fedot_project_root(), file_path, 'women_clothing_review_train.csv')
    test_path = Path(fedot_project_root(), file_path, 'women_clothing_review_test.csv')
    fit_data = MultiModalData.from_csv(file_path=train_path, task=task, index_col=None,
                                       text_columns=['Title', 'Review Text'],
                                       columns_to_drop=['Unnamed: 0', 'Clothing ID'],
    ...
    for data_source in fit_data:
        ...
        automl_model.current_pipeline.save(f'pipeline_cloth/pipeline_cloth_{data_source}', is_datetime_in_path=True)
        history_name = data_source.replace('/', '_')
        automl_model.history.save(f'pipeline_cloth/pipeline_cloth_{history_name}.json')
        print(f'R2 for {data_source} is {round(metrics["r2"], 3)}')

    final_pipeline = update_pipeline(pipelines_dict)
    automl_model = Fedot(problem=task, timeout=5, n_jobs=6, safe_mode=False,
    ...
    automl_model.current_pipeline.save(f'pipeline_cloth', is_datetime_in_path=True)
    automl_model.history.save(f'pipeline_cloth/pipeline_cloth.json')
...
if __name__ == '__main__':
    ...
    metric = run_multi_modal_example(file_path='examples/data/women_clothing_review', is_visualise=True, timeout=30)
