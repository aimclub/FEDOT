...
def get_pipeline(fit_data: MultiModalData):
    if fit_data.data_type[0] is DataTypesEnum.text:
        ...
        # if node_name == 'data_source_text/question_title':
...
def run_multi_modal_example(file_path: str, is_visualise=True, timeout=10) -> float:
    ...
    """
    task = 'regression'
    train_path = Path(fedot_project_root(), file_path, 'google_qa_question_type_reason_explanation_train.csv')
    test_path = Path(fedot_project_root(), file_path, 'google_qa_question_type_reason_explanation_test.csv')
    fit_data = MultiModalData.from_csv(file_path=train_path, task=task, index_col=None,
                                       ...
                                       target_columns='question_type_reason_explanation')
    ...
    for data_source in fit_data:
        ...
        automl_model.current_pipeline.save(f'pipeline_qaq/pipeline_qaq{data_source}', is_datetime_in_path=True)
        history_name = data_source.replace('/', '_')
        # automl_model.history.save(f'pipeline_qaq/pipeline_qaq{history_name}.json')
    ...
    # automl_model.history.save(f'pipeline_qaq/pipeline_qaq.json')
    return metrics["r2"]


if __name__ == '__main__':
    metric = run_multi_modal_example(file_path='examples/data/google_qa_question_type_reason_explanation',
                                     is_visualise=True, timeout=10)
