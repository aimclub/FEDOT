...
def update_pipeline(pipeline: dict) -> Pipeline:
    ...
    for node in pipeline['data_source_text/Author'].nodes:
        final_pipeline.add_node(node)
    final_pipeline.connect_nodes(final_pipeline.nodes[pos_ens_node
                                                      + len(pipeline['data_source_text/Title'].nodes) + 1],
                                 final_pipeline.nodes[pos_ens_node])
    for node in pipeline['data_source_text/Edition'].nodes:
        final_pipeline.add_node(node)
    final_pipeline.connect_nodes(final_pipeline.nodes[pos_ens_node
                                                      + len(pipeline['data_source_text/Title'].nodes)
                                                      + len(pipeline['data_source_text/Author'].nodes) + 1],
                                 final_pipeline.nodes[pos_ens_node])
    for node in pipeline['data_source_text/Synopsis'].nodes:
        final_pipeline.add_node(node)
    final_pipeline.connect_nodes(final_pipeline.nodes[pos_ens_node
                                                      + len(pipeline['data_source_text/Title'].nodes)
                                                      + len(pipeline['data_source_text/Author'].nodes)
                                                      + len(pipeline['data_source_text/Edition'].nodes) + 1],
                                 final_pipeline.nodes[pos_ens_node])
    for node in pipeline['data_source_text/Genre'].nodes:
        final_pipeline.add_node(node)
    final_pipeline.connect_nodes(final_pipeline.nodes[pos_ens_node
                                                      + len(pipeline['data_source_text/Title'].nodes)
                                                      + len(pipeline['data_source_text/Author'].nodes)
                                                      + len(pipeline['data_source_text/Edition'].nodes)
                                                      + len(pipeline['data_source_text/Synopsis'].nodes) + 1],
                                 final_pipeline.nodes[pos_ens_node])
...
def run_multi_modal_example(file_path: str, is_visualise=True, timeout=10) -> float:
    ...
    """
    task = 'regression'
    train_path = Path(fedot_project_root(), file_path, 'bookprice_prediction_train.csv')
    test_path = Path(fedot_project_root(), file_path, 'bookprice_prediction_train.csv')
    fit_data = MultiModalData.from_csv(file_path=train_path, task=task, index_col=None,
                                       text_columns=['Title', 'Author', 'Edition', 'Synopsis', 'Genre'],
    ...
    for data_source in fit_data:
        ...
        automl_model.fit(features=fit_data_source,
                         ...
                         predefined_model=get_pipeline(fit_data_source))
    ...
    automl_model.fit(features=fit_data,
                     ...
                     predefined_model=final_pipeline)
    prediction = automl_model.predict(predict_data)
    metrics = automl_model.get_metrics(metric_names=['r2'])
    automl_model.current_pipeline.show(engine='pyvis')
    r2 = round(metrics["r2"], 3)
    automl_model.current_pipeline.save(f'pipeline_book', is_datetime_in_path=True)
    # automl_model.history.save(f'pipeline_book/pipeline_book.json')
    print(f'R2 for final model is {r2}')
    return metrics["r2"]


if __name__ == '__main__':
    metric = run_multi_modal_example(file_path='examples/data/bookprice_prediction', is_visualise=True, timeout=30)
