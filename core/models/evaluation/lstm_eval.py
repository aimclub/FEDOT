...
def fit_lstm(train_data: InputData):
    ...
    model.fit(train_data.features, train_data.target, epochs=1000,
              callbacks=[
                  tf.keras.callbacks.ModelCheckpoint('lstm_model.h5', monitor='loss', save_best_only=True),
                  tf.keras.callbacks.EarlyStopping(monitor='loss', min_delta=percent, patience=20),
                  tf.keras.callbacks.ReduceLROnPlateau(monitor='loss', factor=0.2),
                  tf.keras.callbacks.TensorBoard(update_freq=train_data.features.shape[0] // 10)
              ])
