...
For example, ``PresetsEnum.BEST_QUALITY`` option allows FEDOT to use entire list of available models for a specified task.
In contrast ``PresetsEnum.FAST_TRAIN`` ensures only fast learning models are going to be used.

Apart from that there are other options whose names speak for themselves: ``PresetsEnum.STABLE``, ``PresetsEnum.AUTO``,
``PresetsEnum.GPU``, ``PresetsEnum.TS``, ``PresetsEnum.AUTO_ML`` (the latter uses only AutoML models as pipeline nodes).

.. note::
    To make it simple, FEDOT uses ``PresetsEnum.AUTO`` by default to identify the best choice for you.
