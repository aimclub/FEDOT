...
def test_tensor_data_merger_concatenates_parent_features(base_tensor_data):
    branch_a = replace(
        base_tensor_data, features=base_tensor_data.features[:, :1] * 10)
    branch_b = replace(
        base_tensor_data, features=base_tensor_data.features[:, 1:2] + 100)
...
def test_tensor_data_merger_uses_branch_with_target_as_main(base_tensor_data):
    ...
    merged = TensorDataMerger(
        [branch_without_target, branch_with_target]).merge()
...
def test_tensor_data_merger_filters_by_common_idx(base_tensor_data):
    ...
    assert torch.allclose(
        merged.features[:, 0], torch.tensor([20.0, 30.0, 40.0]))
    assert torch.allclose(
        merged.features[:, 1], torch.tensor([101.0, 102.0, 103.0]))
...
def test_tensor_data_merger_raises_on_no_common_indices(base_tensor_data):
    branch_a = replace(base_tensor_data, idx=np.array([0, 1]),
                       features=base_tensor_data.features[:2], target=base_tensor_data.target[:2])
...
def test_tensor_data_merger_promotes_1d_features_to_2d(base_tensor_data):
    ...
    assert merged.features.shape == (4, 2)
    assert torch.allclose(
        merged.features[:, 0], torch.tensor([10.0, 20.0, 30.0, 40.0]))
    assert torch.allclose(
        merged.features[:, 1], torch.tensor([1.0, 2.0, 3.0, 4.0]))
...
def test_tensor_data_merger_flattens_branches_with_different_ndims(base_tensor_data):
    ...
    assert torch.allclose(
        merged.features[:, 2:], branch_3d.features.reshape(4, -1))
...
def test_tensor_data_merger_flattens_incompatible_same_ndim_shapes(base_tensor_data):
    ...
    assert torch.allclose(
        merged.features[:, :6], branch_a.features.reshape(4, -1))
    assert torch.allclose(
        merged.features[:, 6:], branch_b.features.reshape(4, -1))
...
def test_tensordata_from_parents_merges_stub_parent_outputs(base_tensor_data):
    ...
    parent_a.fit = lambda tensor_data, predictions_cache=None, fold_id=None: replace(
        ...
        predict=torch.full((tensor_data.features.shape[0],), 999.0),
    )
    parent_b.fit = lambda tensor_data, predictions_cache=None, fold_id=None: replace(
        tensor_data,
        features=tensor_data.features[:, 1:2] + 100,
        predict=torch.full((tensor_data.features.shape[0],), 888.0),
    )

    child = PipelineNode('torch_linear', nodes_from=[parent_b, parent_a])
    merged = child._tensordata_from_parents(
        base_tensor_data, parent_operation='fit')

    ordered_parents = [
        node.operation.operation_type for node in child._nodes_from_with_fixed_order()]
    assert ordered_parents == ['normalization', 'scaling']
    assert merged.features.shape == (4, 2)
    assert merged.predict is None
    assert torch.allclose(
        merged.features[:, 0], base_tensor_data.features[:, 1] + 100)
    assert torch.allclose(
        merged.features[:, 1], base_tensor_data.features[:, 0] * 10)
