...
def test_build_industrial_task_plan_uses_detection_window_for_predict_horizon():
    plan = build_industrial_task_plan('classification', {'data_type': 'time_series', 'detection_window': 7})
...
def test_build_industrial_task_plan_builds_regular_classification_task():
    plan = build_industrial_task_plan('classification', {'data_type': 'tensor'})
...
def test_resolve_learning_strategy_flags_detects_big_and_tabular_contexts():
    flags = resolve_learning_strategy_flags({'learning_strategy': 'ts2tabular_big'})
...
def test_should_use_predict_horizon_requires_time_series_and_window():
    assert should_use_predict_horizon({'data_type': 'time_series', 'detection_window': 5}) is True
