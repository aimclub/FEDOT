...
def test_api_params_normalizes_timeout_and_generations():
    params = ApiParams({'num_of_generations': 5}, problem='classification', timeout=-1)

    assert params.timeout is None
    assert params['num_of_generations'] == 5

    params_with_default_generations = ApiParams({}, problem='classification', timeout=1)
...
def test_api_params_accept_and_apply_recommendations_updates_internal_mapping(monkeypatch):
    ...
    monkeypatch.setattr(params, 'change_preset_for_label_encoded_data', fake_change_preset)

    params.accept_and_apply_recommendations(
        input_data=type('Data', (), {'task': params.task, 'data_type': None})(),
